import { Clock3, Disc3, Home, Library, ListMusic, Mic2, Music, Search, Sparkles } from 'lucide-react'
import { Link, NavLink, useLocation } from 'react-router-dom'
import { useLibraryPlaylists } from '../../hooks/useCatalog'
import { resolveMediaUrl } from '../../lib/media'

const browseItems = [
  { to: '/', label: 'Home', Icon: Home },
  { to: '/search', label: 'Search', Icon: Search },
  { to: '/new', label: 'New', Icon: Sparkles },
]

const libraryItems = [
  { to: '/library?section=recent', label: 'Recently added', Icon: Clock3, section: 'recent' },
  { to: '/library?section=artists', label: 'Artists', Icon: Mic2, section: 'artists' },
  { to: '/library?section=albums', label: 'Albums', Icon: Disc3, section: 'albums' },
  { to: '/library?section=songs', label: 'Songs', Icon: Music, section: 'songs' },
]

export default function Sidebar() {
  const { data: playlists } = useLibraryPlaylists()
  const location = useLocation()
  const activeLibrarySection = new URLSearchParams(location.search).get('section')

  return (
    <aside className="flex w-64 shrink-0 flex-col gap-3">
      <nav className="glass-panel-vibrant soft-enter rounded-[28px] px-3 py-4">
        <p className="px-3 pb-2 text-xs font-semibold tracking-wide uppercase text-muted-fg">Browse</p>
        <ul className="space-y-0.5">
          {browseItems.map(({ to, label, Icon }) => (
            <li key={to}>
              <NavLink
                to={to}
                end={to === '/'}
                className={({ isActive }) =>
                  `surface-hover flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-semibold ${
                    isActive
                      ? 'bg-accent/12 text-foreground shadow-sm'
                      : 'text-muted-fg hover:bg-foreground/5 hover:text-foreground'
                  }`
                }
              >
                <Icon size={20} />
                {label}
              </NavLink>
            </li>
          ))}
        </ul>
      </nav>

      <nav className="glass-panel-vibrant soft-enter rounded-[28px] px-3 py-4">
        <div className="px-3 pb-2 flex items-center gap-2">
          <Library size={18} className="text-muted-fg" />
          <Link
            to="/library"
            className="text-xs font-semibold tracking-wide uppercase text-muted-fg hover:text-foreground transition-colors"
          >
            Library
          </Link>
        </div>
        <ul className="space-y-0.5">
          {libraryItems.map(({ to, label, Icon, section }) => {
            const isActive = location.pathname === '/library' && activeLibrarySection === section
            return (
              <li key={to}>
                <Link
                  to={to}
                  className={`surface-hover flex items-center gap-3 rounded-2xl px-3 py-2.5 text-sm font-semibold ${
                    isActive
                      ? 'bg-accent/12 text-foreground shadow-sm'
                      : 'text-muted-fg hover:bg-foreground/5 hover:text-foreground'
                  }`}
                >
                  <Icon size={18} />
                  {label}
                </Link>
              </li>
            )
          })}
        </ul>
      </nav>

      <div className="glass-panel-vibrant soft-enter flex flex-1 flex-col overflow-hidden rounded-[28px]">
        <div className="px-4 pt-4 pb-2 flex items-center gap-2">
          <ListMusic size={18} className="text-muted-fg" />
          <Link
            to="/playlists"
            className="text-xs font-semibold tracking-wide uppercase text-muted-fg hover:text-foreground transition-colors"
          >
            Playlists
          </Link>
        </div>

        <div className="flex-1 overflow-y-auto px-2 pb-3">
          {playlists && playlists.items.length > 0 ? (
            <ul className="space-y-0.5">
              {playlists.items.map((playlist) => {
                const coverUrl = resolveMediaUrl(playlist.coverUrl)

                return (
                  <li key={playlist.id}>
                    <NavLink
                      to={`/playlist/${playlist.id}`}
                      className={({ isActive }) =>
                        `surface-hover flex items-center gap-3 rounded-2xl px-3 py-2.5 ${
                          isActive
                            ? 'bg-accent/10 text-foreground shadow-sm'
                            : 'text-muted-fg hover:bg-foreground/5 hover:text-foreground'
                        }`
                      }
                    >
                      <div className="w-9 h-9 rounded-xl bg-surface/70 shrink-0 overflow-hidden">
                        {coverUrl ? (
                          <img src={coverUrl} alt={playlist.name} className="w-full h-full object-cover" />
                        ) : (
                          <div className="w-full h-full flex items-center justify-center">
                            <ListMusic size={14} className="text-muted-fg" />
                          </div>
                        )}
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium truncate text-foreground">{playlist.name}</p>
                        <p className="text-xs text-muted-fg truncate">
                          {playlist.isAiCurated ? 'AI Curated' : 'Library playlist'}
                        </p>
                      </div>
                    </NavLink>
                  </li>
                )
              })}
            </ul>
          ) : (
            <p className="px-3 text-xs text-muted-fg">No library playlists yet.</p>
          )}
        </div>
      </div>
    </aside>
  )
}
